...
- [ ] **1.1** Add "Are you an agency?" link on login + signup screens
  - Files: `login_screen.dart`, `signup_screen.dart`
  - Link below existing sign-in/sign-up text → navigates to `/agency-login`
- [ ] **1.2** Build Agency Login Screen — web-optimized, responsive
  - File: `lib/src/features/agency_dashboard/presentation/screens/agency_login_screen.dart` (NEW)
  - Reference: `design_mockups/00_agency_login.html`
  - Desktop (> 900px): two-column split — hero image left, login card right (maxWidth: 400px)
  - Mobile (< 900px): stacked — hero on top (250px), form below
...
- [ ] **1.3** Build Agency Signup Screen — Step 1: Company Details
  - File: `screens/agency_signup_screen.dart` (NEW)
  - Reference: `design_mockups/01_agency_signup.html`
  - Fields: business name, email, phone, description, specialties, team size, experience
- [ ] **1.4** Build Signup Step 2: Document Upload
  - GST certificate + portfolio photos (up to 5)
  - Firebase Storage integration
- [ ] **1.5** Build Signup Step 3: Bank Details
  - Account holder, account number, IFSC, bank name
- [ ] **1.6** Build Agency Pending Screen
  - File: `screens/agency_pending_screen.dart` (NEW)
  - Reference: `design_mockups/02_agency_pending.html`
  - "Under review" message + logout button
  - Listens to verificationStatus stream, auto-redirect on approval
- [ ] **1.7** Add `registerAgency()` to AuthRepository
  - Creates agency doc (pending) + updates user (role, agencyId)
  - File: `lib/src/features/authentication/data/auth_repository.dart`
- [ ] **1.8** Add routes: `/agency-login`, `/agency-signup`, `/agency-pending` in app_router.dart
- [ ] **1.9** Verify: "Are you an agency?" → agency login → signup → pending screen
- [ ] **1.10** Verify: existing consumer login still works unchanged
- [ ] **1.11** Verify: pending user cannot access /agency dashboard

**Checkpoint:** Full agency login + registration flow working. Web-optimized.
...
- [ ] **2.1** Update `AppUser` model — add `@Default('consumer') String role` + `String? agencyId`
  - File: `lib/src/features/authentication/domain/app_user.dart`
- [ ] **2.2** Run `build_runner` — regenerate Freezed + JSON
  - Command: `dart run build_runner build --delete-conflicting-outputs`
- [ ] **2.3** Create `Agency` model (Freezed) matching `database_schema.md`
  - File: `lib/src/features/agency/domain/agency.dart` (NEW)
- [ ] **2.4** Run `build_runner` again for Agency model
- [ ] **2.5** Create `AgencyRepository` with CRUD + Riverpod providers
...
- [ ] **2.6** Add role-based redirect to `GoRouter`
  - File: `lib/src/routing/app_router.dart`
  - Logic: agency + approved → `/agency`, agency + pending → `/agency-pending`, superAdmin → `/admin`, consumer → `/`
- [ ] **2.7** Add new `AppRoute` enum entries (agencyLogin, agencySignup, agencyPending, agencyHome, agencyTrips, etc.)
  - File: `lib/src/routing/app_router.dart`
...
- [ ] **3.1** Create `DashboardSidebar` component
...
- [ ] **3.6** Create 4 placeholder screens (empty Scaffold with title)
...
- [ ] **3.7** Wire `StatefulShellRoute` for agency in `app_router.dart`
  - Branches: /agency, /agency/trips, /agency/bookings, /agency/profile
- [ ] **3.8** Verify: resize browser → sidebar ↔ rail ↔ bottom nav transitions
- [ ] **3.9** Verify: all 4 tabs navigate correctly, selected state highlights
...
- [ ] **4.5** Verify: agency sees real stats from Firestore
- [ ] **4.6** Verify: agency can edit profile, changes persist
...
- [ ] **5.5** Create Trip Wizard — Step 1: Basic Info
...
- [ ] **5.6** Create Trip Wizard — Step 2: Media
...
- [ ] **5.11** Build Agency Add Trip Screen (orchestrates wizard)
...
- [ ] **5.13** Add routes for add/edit trip in app_router.dart
- [ ] **5.14** Verify: create trip via wizard → appears in consumer Explore feed
- [ ] **5.15** Verify: edit existing trip → changes reflected
...
- [ ] **6.1** Add `getBookingsForTrip(tripId)` to BookingRepository
  - File: `lib/src/features/bookings/data/booking_repository.dart`
- [ ] **6.2** Build Agency Bookings Screen
...
- [ ] **6.4** Add route for trip bookings in app_router.dart
- [ ] **6.5** Verify: agency views all bookings across trips
- [ ] **6.6** Verify: filter by status works
- [ ] **6.7** Verify: per-trip booking view shows correct data
...
|-------|-------|------|--------|
| 1. Agency Signup & Login | 11 | 0 | Not started |
| 2. Foundation | 9 | 0 | Not started |
| 3. Dashboard Shell | 9 | 0 | Not started |
| 4. Home + Profile | 6 | 0 | Not started |
| 5. Trip Management | 15 | 0 | Not started |
| 6. Booking Management | 7 | 0 | Not started |
